$(function () {
    ...
    function showTrainedInfo(info) {
        var formatStatus = function (cellValue) {
            if (cellValue === true) return "Многослойный персептрон";
    ...
    function validateUserModel() {
        ...
        //     msg.html('Количество нейронов на скрытом слое для сети радиально-базисных функций должно быть больше 0');
        //     block.show();
        //     return false;
        // }
        //
        // if (mlpNum.val() !== "" && parseInt(mlpNum) < 1) {
        //     msg.html('Количество нейронов на скрытом слое для многослойного персептрона должно быть больше 0');
        //     block.show();
        //     return false;
        // }
    ...
    function validate() {
        ...
        if (mlp.prop("checked") && mlpMinNum.val() === "") {
            msg.html("Минимальное число нейронов скрытого слоя для многослойного персептрона не заполнено");
            errorBlock.show();
            return false;
        }

        if (mlp.prop("checked") && mlpMaxNum.val() === "") {
            msg.html("Максимальное число нейронов скрытого слоя для многослойного персептрона не заполнено");
            errorBlock.show();
            return false;
        }
        ...
        if (mlp.prop("checked") && parseInt(mlpMinNum.val()) < 1) {
            msg.html("Минимальное число нейронов скрытого слоя для многослойного персептрона должно быть больше 0");
            errorBlock.show();
            return false;
        }

        if (mlp.prop("checked") && parseInt(mlpMaxNum.val()) < 1) {
            msg.html("Максимальное число нейронов скрытого слоя для многослойного персептрона должно быть больше 0");
            errorBlock.show();
            return false;
        }

        if (rbf.prop("checked") && rbfMinNum.val() === "") {
            msg.html("Минимальное число нейронов для сети радиально-базисных функций не заполнено");
            errorBlock.show();
            return false;
        }

        if (rbf.prop("checked") && rbfMaxNum.val() === "") {
            msg.html("Максимальное число нейронов для сети радиально-базисных функций не заполнено");
            errorBlock.show();
            return false;
        }
        ...
        if (rbf.prop("checked") && parseInt(rbfMinNum.val()) < 1) {
            msg.html("Минимальное число нейронов скрытого слоя для сети радиально-базисных функций должно быть больше 0");
            errorBlock.show();
            return false;
        }

        if (rbf.prop("checked") && parseInt(rbfMaxNum.val()) < 1) {
            msg.html("Максимальное число нейронов скрытого слоя для сети радиально-базисных функций должно быть больше 0");
            errorBlock.show();
            return false;
        }
